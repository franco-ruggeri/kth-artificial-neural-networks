...
class RestrictedBoltzmannMachine:
    ...
    def cd1(self, visible_trainset, n_epochs=20):
        ...
        np.random.shuffle(visible_trainset)                 # randomize
        batches = self._get_minibatches(visible_trainset)   # mini-batches
        self.reconstruction_error = [self.compute_reconstruction_error(visible_trainset)]
        ...
        for epoch in range(n_epochs):
            for batch in batches:
                ...
                # negative phase
                p_v_1, v_1 = self.get_v_given_h(h_0)        # notice that we use binary samples
                p_h_1, h_1 = self.get_h_given_v(p_v_1)      # notice that we use probabilities

                # [TODO TASK 4.1] update the parameters using function 'update_params'
                self.update_params(v_0, h_0, p_v_1, p_h_1)  # binary samples for h_0, probabilities otherwise
            ...
            if epoch % self.rf["period"] == 0 and self.is_bottom:
                viz_rf(weights=self.weight_vh[:, self.rf["ids"]].reshape((self.image_size[0], self.image_size[1], -1)),
                       epoch=epoch, grid=self.rf["grid"])

            # print progress
            if epoch % self.print_period == 0:
                self.reconstruction_error.append(self.compute_reconstruction_error(visible_trainset))
                print ("epoch=%2d recon_loss=%4.4f" % (epoch, self.reconstruction_error[-1]))
    ...
    def update_params(self, v_0, h_0, v_k, h_k):
        ...
        positive = v_0.T.dot(h_0)
        negative = v_k.T.dot(h_k)
    ...
    def get_h_given_v(self, visible_minibatch):
        ...
        p_h = sigmoid(self.bias_h + visible_minibatch.dot(self.weight_vh))
        h = sample_binary(p_h)
        return p_h, h
    ...
    def get_v_given_h(self, hidden_minibatch):
        ...
        if self.is_top:
            ...
            p_v = sigmoid(self.bias_v + hidden_minibatch.dot(self.weight_vh.T))
            v = sample_binary(p_v)
    ...
    def gibbs(self, visible_data):
        """Perform one Gibbs sampling step."""
        h = self.get_h_given_v(visible_data)[1]
        v = self.get_v_given_h(h)[1]
        return v

    def compute_reconstruction_error(self, visible_data):
        """Compute the average reconstruction error."""
        reconstructions = self.gibbs(visible_data)
        n_patterns = len(visible_data)
    ...
    def _get_minibatches(self, training_set):
        ...
        for n in range(n_batches):
            start_idx = n * self.batch_size
            end_idx = (n+1) * self.batch_size
            batches.append(training_set[start_idx:end_idx])
        return batches
